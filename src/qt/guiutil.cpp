...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a EZY address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no ezy: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert ezy:// to ezy:
    //
    //    Cannot handle this later, because ezy:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ezy.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EZY.lnk";
}
...
{
    return GetAutostartDir() / "ezy.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ezy.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ezy app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ezy app to startup item list
