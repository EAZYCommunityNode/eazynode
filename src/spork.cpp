...
// EZY: on startup load spork values from previous session if they exist in the sporkDB
...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    CPubKey pubkeynew( 
        ParseHex( 
            spork.nTimeSigned >= Params().RejectOldSporkKey() 
                ? Params().SporkKey() 
                : Params().SporkKeyOld()
        )
    );
